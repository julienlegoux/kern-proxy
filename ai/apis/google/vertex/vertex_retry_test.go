...
// Proves the vertex adapter honors StreamOptions.MaxRetries/MaxRetryDelay
// through the shared request-level retry loop (ai/apis/internal/httpretry).
// Mirrors the anthropic, openaicompletions, openairesponses, azure, and codex
// retry tests.
...
func retryServer(t *testing.T, attempts *int32, respond func(w http.ResponseWriter, attempt int32) bool) *httptest.Server {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(writeRetrySSE(successChunks())))
	}))
	t.Cleanup(srv.Close)
	return srv
}

func retryOpts(opts *ai.StreamOptions) *ai.StreamOptions {
	opts.APIKey = "AIzaSyExampleRealisticLookingApiKey123456"
	return opts
}
...
func streamResult(t *testing.T, srv *httptest.Server, opts *ai.StreamOptions) *ai.AssistantMessage {
	...
	defer cancel()
	stream := Stream(ctx, testModel(srv.URL), retryChat(), retryOpts(opts))
...
// TestStream_QuotaErrorIsNotRetried uses the real shape of a Gemini free-tier
// 429: RESOURCE_EXHAUSTED with "quota exceeded" in the message. Retrying it
// only burns the caller's backoff budget on an account-level limit.
func TestStream_QuotaErrorIsNotRetried(t *testing.T) {
	var attempts int32
	srv := retryServer(t, &attempts, func(w http.ResponseWriter, _ int32) bool {
		w.Header().Set("content-type", "application/json")
		w.WriteHeader(http.StatusTooManyRequests)
		_, _ = w.Write([]byte(`{"error":{"code":429,"status":"RESOURCE_EXHAUSTED","message":"Quota exceeded for metric: generate_content_free_tier_requests"}}`))
		return true
	})
	...
	if got := atomic.LoadInt32(&attempts); got != 1 {
		t.Fatalf("attempts = %d, want 1 (quota exhaustion is terminal)", got)
	}
}
...
func TestStream_ContextCancellationAbortsPendingBackoff(t *testing.T) {
	...
	maxRetries := 5
	stream := Stream(ctx, testModel(srv.URL), retryChat(), retryOpts(&ai.StreamOptions{MaxRetries: &maxRetries}))
