...
// Proves the azure adapter honors StreamOptions.MaxRetries/MaxRetryDelay
// through the shared request-level retry loop (ai/apis/internal/httpretry).
// Mirrors the openairesponses retry tests, whose stream core this adapter
// delegates to.
...
func writeAzureSSE(events []string) string {
...
func retryServer(t *testing.T, attempts *int32, respond func(w http.ResponseWriter, attempt int32) bool) *httptest.Server {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(writeAzureSSE(minimalTextEvents())))
	}))
	t.Cleanup(srv.Close)
	return srv
}
...
func retryOpts(opts *ai.StreamOptions) *ai.StreamOptions {
	opts.APIKey = "azure-key"
	opts.AzureDeploymentName = "my-deployment"
	return opts
}
