...
// Proves the openaicompletions adapter honors
// StreamOptions.MaxRetries/MaxRetryDelay through the shared request-level
// retry loop (ai/apis/internal/httpretry). Mirrors the anthropic and codex
// retry tests.
...
func TestStream_RetryAfterIsHonoredAndClampedByMaxRetryDelay(t *testing.T) {
	...
	if elapsed > 2*time.Second {
		t.Fatalf("elapsed = %v: MaxRetryDelay did not clamp the 1h retry-after-ms", elapsed)
	}
	if got := atomic.LoadInt32(&attempts); got != 2 {
		t.Fatalf("attempts = %d, want 2", got)
	}
...
func TestStream_ContextCancellationAbortsPendingBackoff(t *testing.T) {
	...
	httpretry.BaseDelay = 30 * time.Second
